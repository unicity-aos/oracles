...
- `.mcp.json`, which attaches `aos --principal codex-code mcp attach` to the
  current project workspace; and
...
hooks, and MCP tools are discovered together. The SessionStart hook performs a
short `aos mcp ready --format hook` check; it does not provision the runtime.
